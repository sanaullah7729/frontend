...
const handleAdminAction = async (userId, createdAt, status) => {
  try {
    ...
    if (response.ok) {
      const actionMessages = {
        approved: 'Feedback approved successfully!',
        rejected: 'Feedback rejected successfully!',
        flagged: 'Feedback flagged for review!'
      };
      showNotification('success', actionMessages[status]);
      await fetchFeedback();
    } else {
      showNotification('error', `Failed to ${status} feedback`);
    }
  } catch (error) {
    console.error(`Error ${status}ing feedback:`, error);
    showNotification('error', 'Error connecting to server');
  }
};
  ...
  // Delete feedback
const deleteFeedback = async (userId, createdAt) => {
              ...
              <div className="space-y-6">
                {filteredFeedback.length === 0 ? (
                  ...
                  filteredFeedback.map((item) => (
                    <div key={`${item.userId}-${item.createdAt}`} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="flex items-center space-x-1 mr-2">
                            <button
                              onClick={() => handleAdminAction(item.userId, item.createdAt, 'approved')}
                            ...
                            <button
                              onClick={() => handleAdminAction(item.userId, item.createdAt, 'rejected')}
                            ...
                            <button
                              onClick={() => handleAdminAction(item.userId, item.createdAt, 'flagged')}
                          ...
                          <button
                            onClick={() => deleteFeedback(item.userId, item.createdAt)}
                            className="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors"
                            title="Delete"
                          >
